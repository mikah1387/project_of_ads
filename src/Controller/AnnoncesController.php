...
class AnnoncesController extends AbstractController
{
  ...
  public function index(Annonces $annonce, Request $request, SendMailService $mailer, EntityManagerInterface $em, CommentsRepository $commentRepo, CacheInterface $cache): Response
  {

    // $annonce = $cache->get('annonce_detail_' . $annonces->getSlug(), function (ItemInterface $item) use ($annonces) {
    //   $item->expiresAfter(20);
    //   return $annonces;
    // });

    // exmple de cache :
    //  $text = $cache->get('test_',function(ItemInterface $item){
    //   $item->expiresAfter(20);
